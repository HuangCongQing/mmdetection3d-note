...
class SeparateHead(BaseModule):
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            if head == 'height':
                ret_dict[head] = self.__getattr__(head)(x).sum(dim=1,keepdim=True)
            else:
...
class CenterHead(BaseModule):
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        ...
        for idx, task_head in enumerate(self.task_heads):
            ...
            #                                   dtype=torch.float32)
            anno_box = gt_bboxes_3d.new_zeros((max_objs, 8), # for kitti dataset
                                              dtype=torch.float32)
    ...
    def get_task_detections(self, num_class_with_bg, batch_cls_preds,
        ...
        for i, (box_preds, cls_preds, cls_labels) in enumerate(
            ...
            if top_scores.shape[0] != 0:
                ...
                print('code size is:', self.bbox_coder.code_size)
